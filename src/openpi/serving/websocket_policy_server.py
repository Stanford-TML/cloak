    ...
    async def _handler(self, websocket: _server.ServerConnection):
        ...
        while True:
            try:
                ...
                # Out-of-band control messages from the client (no inference,
                # no action returned). Currently used to flush the debug
                # recorder at episode boundaries while keeping the websocket
                # session open across episodes.
                if isinstance(obs, dict) and obs.get("_control") == "episode_done":
                    # Drop IK warm-starts so the next episode's first solve
